...
document.getElementById('simpriz').innerHTML = `
* {
...
#menu:hover #search-box, #menu2:hover #search-box {
	display: grid;
}
#Image1 > .widget-content > a {
...
#FeaturedPost1 #post-summary-child {
	display: flex;
	flex-direction: column;
	justify-content: center;
	row-gap: 2vw;
...
#Blog1 .index-thumbnail {
	display: grid;
}
#Blog1 .index-thumbnail img {
	width: 100%;
}
#Blog1 .index-summary {
	display: flex;
...
#Blog1 .index-summary > div > span {
	font-size: 12px;
	font-weight: bold;
	color: #eee;
}
#Blog1 .index-summary > div > span > a {
	color: #999;
}
#Blog1 .index-summary > div > span > i {
	color: #999;
}
#Blog1 .button-read-more {
	padding: 5px 15px;
	background: #aaa;
	color: #1B262C;
	font-size: 16px;
...
.article-item p, .article-item li, .article-item td, .article-item ul {
	color: #444;
	font-size: 18px;
	line-height: 150%;
	margin: 18px 0;
...
#PopularPosts1 .popular-posts > ul {
	display: grid;
	row-gap: 10px;
	grid-template-columns: auto;
}
...
.fa-solid.fa-circle-chevron-up::after {
	content: "";
	display: block;
	height: 30px;
	width: 30px;
